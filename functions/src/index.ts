...
import {fetchImageByKeyword} from "./unsplash";

initializeApp();
const db = getFirestore();

export const scheduledAutoPostMorning = onSchedule(
  {
    schedule: "0 9 * * *", // 毎朝9:00
...
export const scheduledAutoPostEvening = onSchedule(
  {
    schedule: "0 18 * * *", // 毎夕18:00
    secrets: [OPENAI_API_KEY],
    timeZone: "Asia/Tokyo",
  },
  async () => {
...
async function generateAndPost() {
  ...
  async function getRandomKeyword(): Promise<string | null> {
    ...
    recentSnap.docs.forEach((doc) => {
      const data = doc.data();
      if (data?.keyword) recentKeywords.add(data.keyword);
    });

    const filtered = allKeywords.filter(
      (k) => !recentKeywords.has(k.keyword)
    );
